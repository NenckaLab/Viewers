...
function fillSegmentation(segmentation, inputLabelmaps3D, userOptions = {}) {
    ...
    const labelmaps3D = Array.isArray(inputLabelmaps3D) ?
        inputLabelmaps3D : [inputLabelmaps3D];

    let numberOfFrames = 0;
    const referencedFramesPerLabelmap = [];

    for (
        let labelmapIndex = 0; labelmapIndex < labelmaps3D.length; labelmapIndex++
    ) {
        ...
        for (let i = 0; i < labelmaps2D.length; i++) {
            ...
            if (labelmaps2D[i]) {
                ...
                segmentsOnLabelmap.forEach(segmentIndex => {
                    if (segmentIndex !== 0) {
                        // Ensure the array exists for this segment index
                        if (!referencedFramesPerSegment[segmentIndex]) {
    ...
    for (
        ...
        for (
            let segmentIndex = 1; segmentIndex < referencedFramesPerSegment.length; segmentIndex++
        ) {
            ...
            if (referencedFrameIndicies) {
                ...
                let segmentMetadata = metadata[segmentIndex];

                // If metadata doesn't exist for this segment, create default metadata
                if (!segmentMetadata) {
                    segmentMetadata = {
                        ...
                        RecommendedDisplayCIELabValue: [255, 0, 0, 255], // Default red
...
function generateToolState(
    ...
    if (TransferSyntaxUID === '1.2.840.10008.1.2.5') {
        const rleEncodedFrames = Array.isArray(multiframe.PixelData) ?
            multiframe.PixelData : [multiframe.PixelData];
    ...
    const orientation = checkOrientation(
        multiframe,
        validOrientations, [imagePlaneModule.rows, imagePlaneModule.columns, imageIds.length],
        tolerance
    );
...
function insertPixelDataPlanar(
    ...
    for (
        let i = 0, groupsLen = PerFrameFunctionalGroupsSequence.length; i < groupsLen;
        ++i
    ) {
        ...
        const pixelDataI2D = ndarray(
            new Uint8Array(pixelData.buffer, i * sliceLength, sliceLength), [Rows, Columns]
        );
...
function insertPixelDataPerpendicular_v0(
    ...
    const rowCosines = Array.isArray(firstImagePlaneModule.rowCosines) ? [...firstImagePlaneModule.rowCosines] : [
        firstImagePlaneModule.rowCosines.x,
        firstImagePlaneModule.rowCosines.y,
        firstImagePlaneModule.rowCosines.z,
    ];

    const columnCosines = Array.isArray(firstImagePlaneModule.columnCosines) ? [...firstImagePlaneModule.columnCosines] : [
...
function insertOverlappingPixelDataPlanar(
    ...
    for (
        let segmentIndexToProcess = 1; segmentIndexToProcess <= numberOfSegs;
